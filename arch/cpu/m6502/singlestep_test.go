...
func runSingleStepCase(t *testing.T, tc ss6502TestCase) bool {
	t.Helper()

	mem := &ssSparseMemory{data: make(map[uint16]uint8, len(tc.Initial.RAM))}
	...
	// Write interrupt vectors to satisfy New()'s validation requirements.
	// The test will overwrite PC directly after construction.
	memory.WriteWord(ResetAddress, tc.Initial.PC)
	memory.WriteWord(NMIAddress, 0xFFFF)
	memory.WriteWord(IrqAddress, 0xFFFF)
