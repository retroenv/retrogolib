...
// Benchmark tests
func BenchmarkMemory_Read8(b *testing.B) {
	logger := log.NewTestLogger(b)
	memory, _ := NewMemory(1024*1024, logger)

	b.ResetTimer()
	for i := range b.N {
		_ = memory.Read8(uint32(i % 1000))
	}
}

func BenchmarkMemory_Write8(b *testing.B) {
	logger := log.NewTestLogger(b)
	memory, _ := NewMemory(1024*1024, logger)

	b.ResetTimer()
	for i := range b.N {
		memory.Write8(uint32(i%1000), uint8(i))
	}
}

func BenchmarkMemory_Read16(b *testing.B) {
	logger := log.NewTestLogger(b)
	memory, _ := NewMemory(1024*1024, logger)

	b.ResetTimer()
	for i := range b.N {
		_ = memory.Read16(uint32(i % 1000))
	}
}

func BenchmarkMemory_Write16(b *testing.B) {
	logger := log.NewTestLogger(b)
	memory, _ := NewMemory(1024*1024, logger)

	b.ResetTimer()
	for i := range b.N {
		memory.Write16(uint32(i%1000), uint16(i))
	}
}

func BenchmarkMemory_SegmentedRead(b *testing.B) {
	logger := log.NewTestLogger(b)
	memory, _ := NewMemory(1024*1024, logger)

	b.ResetTimer()
	for i := range b.N {
		_ = memory.ReadSegmented(0x1000, uint16(i%1000))
	}
}

func BenchmarkMemory_SegmentedWrite(b *testing.B) {
	logger := log.NewTestLogger(b)
	memory, _ := NewMemory(1024*1024, logger)

	b.ResetTimer()
	for i := range b.N {
		memory.WriteSegmented(0x1000, uint16(i%1000), uint8(i))
	}
}
