...
// Package cpu65816 provides SingleStepTests/65816 JSON-based single-step tests.
//
// To run these tests, download the test data:
//
//	make -C testdata cpu65816
//
// Then run: go test -tags singlestep ./arch/cpu/cpu65816/...
package cpu65816
...
func getSingleStepDir(t *testing.T) string {
	...
	if dir := os.Getenv("CPU65816_TESTDATA"); dir != "" {
		if _, err := os.Stat(dir); err != nil {
			t.Skipf("CPU65816_TESTDATA directory not found: %s", dir)
		}
		return dir
	}
	...
	dir := filepath.Join(filepath.Dir(thisFile), "..", "..", "..", "testdata", "cpu65816", "65816", "v1")
	if _, err := os.Stat(dir); os.IsNotExist(err) {
		t.Skipf("SingleStepTests 65816 data not found at %s (run 'make -C testdata cpu65816' to download)", dir)
	}
